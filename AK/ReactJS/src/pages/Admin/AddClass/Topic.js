...
class Class_ extends React.Component {
    constructor(props) {
        super(props);
        this.state = { classes: [], subject: [], chapter: [], topic: [], isOpen: false, selectClassId: '', selectSubjectId: '', cType: '', selectChapterId: '' }
    ...
    saveClass(e) {
        ...
        let data = {
            ...
            course_id: this.state.selectChapterId,
            status: this.statusRef.current.value
        }
        this.props.saveTopic_for_admin(data).then(res => {
            this.setState({
                topic: res,
                isOpen: false
            })
            ...
            <React.Fragment>
                <Grid container spacing={3}>
                    <Grid item xs={12}>
                        ...
                        {this.state.selectChapterId && <Button variant="contained" color="secondary" onClick={this.AddPopup}>
                            Add new Topic
                </Button>}
                    </Grid>
                    <Grid item xs={12}>
                        {this.state.topic.length && this.listTopic()}
